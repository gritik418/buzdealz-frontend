...
import { ShoppingBag, Heart, Menu, Search, User, Bell, Sparkles, LogOut, Clock } from 'lucide-react';
...
export const Navbar = () => {
  ...
  const { wishlist, user, isAuthenticated, logout, unreadCount, markNotificationsAsRead, notifications } = useWishlist();
           ...
           <div className="relative">
              <button 
                ...
                className={cn(
                  ...
                  notifOpen ? "bg-primary-50 text-primary-600" : "text-slate-400 hover:text-primary-600"
                )}
                title="Notifications"
              >
                  ...
                  <>
                    ...
                    >
                      <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
                         <span className="font-black text-slate-900 text-sm uppercase tracking-wider">Alert Center</span>
                         {unreadCount > 0 && <span className="bg-primary-50 text-primary-600 text-[10px] font-black px-2 py-0.5 rounded-full">{unreadCount} New</span>}
                      </div>
                      
                      <div className="max-h-[400px] overflow-y-auto no-scrollbar">
                        {notifications.length > 0 ? (
                          notifications.map((n) => (
                            <div key={n.id} className={cn("px-5 py-4 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors relative", !n.isRead && "bg-primary-50/30")}>
                               {!n.isRead && <div className="absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-primary-500 rounded-full" />}
                          ...
                          <div className="p-10 text-center">
                             ...
                             <p className="text-slate-400 font-bold text-sm">No new price alerts yet.</p>
                          </div>
                        )}
                      </div>
